...
{%      for cp_item in helpers.toList('PFWsense.captiveportal.zones.zone')  %}
...
{# collect enabled #}
{% set shapers = [] %}
{% if helpers.exists('PFWsense.TrafficShaper') %}
{%     if helpers.exists('PFWsense.TrafficShaper.pipes.pipe') %}
{%         for pipe in helpers.toList('PFWsense.TrafficShaper.pipes.pipe') %}
